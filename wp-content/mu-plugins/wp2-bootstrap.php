...
/**
 * Plugin Name: WP2 CLI Bootstrap Controller
 * Description: CLI commands for managing the install lifecycle including activation, deactivation, and uninstallation.
 * Version: 1.0
 * Author: WP2S
 */

namespace WP2\CLI\Bootstrap;

// Define the plugin slug for activation checks
if (!defined('WP2_BOOTSTRAP_TEMPLATE_SLUG')) {
    define('WP2_BOOTSTRAP_TEMPLATE_SLUG', 'wp2-bootstrap/wp2-bootstrap.php');
}
...
class Controller
{
    ...
    public static function init()
    {
        ...
        self::$command_prefix = 'wp2-bootstrap';

        self::$webhook_url = defined('WP2_BOOTSTRAP_WEBHOOK_URL') && WP2_BOOTSTRAP_WEBHOOK_URL
            ? WP2_BOOTSTRAP_WEBHOOK_URL
            : '';
    ...
    public static function run()
    {
        ...
        if (is_plugin_active(WP2_BOOTSTRAP_TEMPLATE_SLUG)) {
    ...
    {
        \WP_CLI::log("Starting plugin activation for: " . WP2_BOOTSTRAP_TEMPLATE_SLUG);

        // Clear cache before starting
        wp_cache_delete('alloptions', 'options');
        \WP_CLI::log("Cache cleared before activation.");

        $is_active = is_plugin_active(WP2_BOOTSTRAP_TEMPLATE_SLUG);
        ...
        $result = \WP_CLI::runcommand(
            "plugin activate " . WP2_BOOTSTRAP_TEMPLATE_SLUG,
        ...
        $is_active_direct = in_array(WP2_BOOTSTRAP_TEMPLATE_SLUG, $active_plugins, true);
        \WP_CLI::log("Direct database check for plugin active state returned: " . ($is_active_direct ? "true" : "false"));

        $is_active_after = is_plugin_active(WP2_BOOTSTRAP_TEMPLATE_SLUG);
    ...
    private static function retry_plugin_activation()
    {
        ...
        for ($attempt = 1; $attempt <= $retries; $attempt++) {
            ...
            \WP_CLI::runcommand("plugin deactivate " . WP2_BOOTSTRAP_TEMPLATE_SLUG, ['return' => true, 'exit_error' => false]);
            \WP_CLI::log("Plugin deactivated before retry.");

            $result = \WP_CLI::runcommand("plugin activate " . WP2_BOOTSTRAP_TEMPLATE_SLUG, ['return' => true, 'exit_error' => false]);
    ...
    {
        \WP_CLI::log("Attempting to deactivate plugin: " . WP2_BOOTSTRAP_TEMPLATE_SLUG);

        // Run deactivation command
        $result = \WP_CLI::runcommand("plugin deactivate " . WP2_BOOTSTRAP_TEMPLATE_SLUG, ['return' => true, 'exit_error' => false]);
        ...
        if ($result) {
            ...
            if (in_array(WP2_BOOTSTRAP_TEMPLATE_SLUG, $active_plugins, true)) {
                $active_plugins = array_diff($active_plugins, [WP2_BOOTSTRAP_TEMPLATE_SLUG]);
    ...
    public static function uninstall()
    {
        ...
        $result = \WP_CLI::runcommand("plugin uninstall " . WP2_BOOTSTRAP_TEMPLATE_SLUG, ['return' => true]);
    ...
    private static function send_notification($data)
    {
        if (empty(self::$webhook_url)) {
            if (defined('WP_DEBUG') && WP_DEBUG) {
                error_log('[WP2 CLI Bootstrap] Webhook URL is not defined.');
            }
            return;
        }
        ...
        if (is_wp_error($response)) {
            if (defined('WP_DEBUG') && WP_DEBUG) {
                error_log('[WP2 CLI Bootstrap] Notification Error: ' . $response->get_error_message());
            }
            // Implement retry logic
            self::retry_notification($data);
        } else {
            if (defined('WP_DEBUG') && WP_DEBUG) {
                error_log('[WP2 CLI Bootstrap] Notification sent successfully.');
            }
        }
    }
    ...
    private static function retry_notification($data, $retries = 3)
    {
        ...
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log('[WP2 CLI Bootstrap] Retry failed, notification not sent.');
        }
    }
}
