...
namespace WP2\New\Daemon;

// Define the plugin slug for activation checks
if (!defined('WP2_NEW_PLUGIN_SLUG')) {
    define('WP2_NEW_PLUGIN_SLUG', 'wp2-new/wp2-new.php');
}
...
class Controller
{
    ...
    public static function init()
    {
        ...
        self::$command_prefix = 'wp2-new';

        self::$webhook_url = defined('WP2_NEW_WEBHOOK_URL') && WP2_NEW_WEBHOOK_URL
            ? WP2_NEW_WEBHOOK_URL
            : '';
    ...
    {
        \WP_CLI::log("Starting plugin activation for: " . WP2_NEW_PLUGIN_SLUG);

        // Clear cache before starting
        wp_cache_delete('alloptions', 'options');
        \WP_CLI::log("Cache cleared before activation.");

        $is_active = is_plugin_active(WP2_NEW_PLUGIN_SLUG);
        ...
        $is_active_direct = in_array(WP2_NEW_PLUGIN_SLUG, $active_plugins, true);
        \WP_CLI::log("Direct database check for plugin active state returned: " . ($is_active_direct ? "true" : "false"));

        $is_active_after = is_plugin_active(WP2_NEW_PLUGIN_SLUG);
    ...
    private static function retry_plugin_activation()
    {
        ...
        for ($attempt = 1; $attempt <= $retries; $attempt++) {
            ...
            \WP_CLI::runcommand("plugin deactivate " . WP2_NEW_PLUGIN_SLUG, ['return' => true, 'exit_error' => false]);
            \WP_CLI::log("Plugin deactivated before retry.");

            $result = \WP_CLI::runcommand("plugin activate " . WP2_NEW_PLUGIN_SLUG, ['return' => true, 'exit_error' => false]);
    ...
    {
        \WP_CLI::log("Attempting to deactivate plugin: " . WP2_NEW_PLUGIN_SLUG);

        // Run deactivation command
        $result = \WP_CLI::runcommand("plugin deactivate " . WP2_NEW_PLUGIN_SLUG, ['return' => true, 'exit_error' => false]);
    ...
    public static function uninstall()
    {
        ...
        $result = \WP_CLI::runcommand("plugin uninstall " . WP2_NEW_PLUGIN_SLUG, ['return' => true]);
