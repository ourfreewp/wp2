...
class Uninstall
{
    ...
    public static function handle()
    {
        try {
            ...
            if (defined('WP_DEBUG') && WP_DEBUG) {
                error_log('[WP2 BLUEPRINT Template] Uninstall completed successfully.');
            }
        } catch (\Exception $e) {
            if (defined('WP_DEBUG') && WP_DEBUG) {
                error_log('[WP2 BLUEPRINT Template] Uninstall error: ' . $e->getMessage());
            }
    ...
    private static function make_final_mods_blockstudio()
    {
        ...
        if (defined('WP2_BLUEPRINT_WEBHOOK_URL')) {
            self::remove_wp_config_option('WP2_BLUEPRINT_WEBHOOK_URL');
        }
    ...
    private static function uninstall_wp_plugin($plugin_file)
    {
        ...
        if (file_exists($plugin_path)) {
            if (!self::delete_directory($plugin_dir)) {
                if (defined('WP_DEBUG') && WP_DEBUG) {
                    error_log("[WP2 BLUEPRINT Template] Failed to delete plugin directory: {$plugin_dir}");
                }
            }
        }
    }
    ...
    private static function get_wp_config_contents()
    {
        ...
        if (!file_exists($config_path) || !is_writable($config_path)) {
            if (defined('WP_DEBUG') && WP_DEBUG) {
                error_log("[WP2 BLUEPRINT Template] Cannot modify wp-config.php. Check file permissions.");
            }
            return false;
        }
        ...
        if ($contents === false) {
            if (defined('WP_DEBUG') && WP_DEBUG) {
                error_log("[WP2 BLUEPRINT Template] Failed to read wp-config.php.");
            }
        }
        return $contents;
    }
    ...
    private static function write_wp_config_contents($contents)
    {
        ...
        if (file_put_contents($config_path, $contents) === false) {
            if (defined('WP_DEBUG') && WP_DEBUG) {
                error_log("[WP2 BLUEPRINT Template] Failed to write changes to wp-config.php.");
            }
            return false;
        }
        return true;
    ...
    private static function update_constant_value($config_file, $constant, $new_value)
    {
        ...
        if ($updated === null) {
            if (defined('WP_DEBUG') && WP_DEBUG) {
                error_log("[WP2 BLUEPRINT Template] Regex replacement failed for {$constant}.");
            }
        }
        return $updated;
    }
