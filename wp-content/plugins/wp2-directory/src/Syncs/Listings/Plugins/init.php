<?php
// File: wp-content/plugins/wp2-directory/src/Syncs/Listings/Plugins/init.php

namespace WP2_Directory\Syncs\Listings\Plugins;
...
class Controller
{
     ...
     * @var string
     */
    private $hook = 'wp2_directory_trigger_plugins_sync';
     ...
     * @var string
     */
    private $callback = 'wp2_directory_execute_plugins_sync';
     ...
     * @var string
     */
    private $catalog_dir = WP_CONTENT_DIR . '/plugins/wp2-directory/src/Catalogs/Plugins';
    ...
    public function handle_sync(): void
    {
        if ($this->check_context()) {
            ...
            $admin_notice = [
                'type'            => 'info',
                'message'         => 'Syncing Plugin Listings...',
                'is_dismissible'  => true,
...
/**
 * Scheduled callback function for plugin sync.
 *
 * This function is hooked to the async action.
 */
function wp2_directory_execute_plugins_sync(): void
{
    $controller = new Controller();
    $controller->execute_sync();
}

add_action('wp2_directory_trigger_plugins_sync', __NAMESPACE__ . '\\wp2_directory_execute_plugins_sync');
